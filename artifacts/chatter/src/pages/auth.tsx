...
export function AuthPage() {
  ...
  const onSubmit = (values: z.infer<typeof authSchema>) => {
    if (isLogin) {
      ...
      registerMutation.mutate({ data: { username: values.username, password: values.password, email: values.email } }, {
        onSuccess: (data) => {
          setAuthToken(data.token);
          setAuthUser(data.user);
          setLocation("/chats");
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="username" {...field} className="bg-secondary/50 border-0" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
